...
type IUsersHandler interface {
	...
	RefressPassport(c *fiber.Ctx) error
...
func (h *usersHandler) GenerateAdminToken(c *fiber.Ctx) error {
	adminToken, err := kawaiiauth.NewKawaiiAuth(string(kawaiiauth.Admin), h.cfg.Jwt(), nil)
	if err != nil {
		return entities.NewResponse(c).Error(
			fiber.ErrInternalServerError.Code,
...
func (h *usersHandler) RefressPassport(c *fiber.Ctx) error {
...
func (h *usersHandler) GetUserProfile(c *fiber.Ctx) error {
	userId := strings.TrimPrefix(c.Params("user_id"), " ")
